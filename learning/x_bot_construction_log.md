...
## 16. 投稿スタイル刷新（2026-04-14）

### 背景と課題

初期設計の9タイプ（framework / fermi / tips / terminology / mistake / industry / question / quote / promo）は、すべて「教える系」の硬い投稿に偏っていた。

**問題点**:
- 教科書的で読まれにくい
- 共感・感情的フックが弱い
- フォロワーとの距離が縮まらない
- エンゲージメント（コメント・RT・保存）を生む設計になっていない

### 改訂の方針

「読者に役立つ情報を届ける」という第一原則は変えず、**届け方（フォーマット・感情フック）を刷新**した。

参考にしたX運用の知見：
- 共感投稿（あるある系）はRTされやすい
- 対比（NG→OK）は保存・引用RTを生む
- 問いかけはコメントを引き出す
- 数字フックは開封率を上げる
- ストーリー形式は読了率が高い
...
| タイプ | 重み | 主なゴール |
|---|---|---|
| relatable（あるある共感） | 15 | RT・共感・フォロワーとの距離感縮小 |
| before_after（NG→OK対比） | 14 | 保存・即実践・「気づき」提供 |
| quiz（問いかけクイズ） | 13 | コメント・エンゲージメント向上 |
| numbered（数字フック） | 13 | 開封率向上・保存促進 |
| question（ケース問題） | 12 | 練習習慣の形成・コメント交流 |
| mini_story（1コマストーリー） | 10 | 読了率向上・感情移入 |
| save_card（保存推奨まとめ） | 10 | 保存数向上・拡散 |
| seasonal（時事連動） | 10 | タイムリー性・今の課題との共鳴 |
| fermi（フェルミ推定） | 8 | 練習習慣・コメント交流 |
| promo（ツール紹介） | 5 | 集客・継続課金転換 |

**削除したタイプとその理由**:
- `framework`: numbered / save_card でより読まれる形式でカバー可能
- `tips`: before_after / relatable に吸収（同じ内容をより効果的な形式で）
- `terminology`: quiz / numbered に吸収（「用語解説」も問いかけや数字フックで届けられる）
- `industry`: question / seasonal に吸収（業界別ケースは実際の練習問題として出す）
- `quote`: mini_story に吸収（格言を物語形式で使うほうが刺さる）

### character.md への反映内容

10タイプの定義・ゴール・フォーマット・トーンを「投稿パターン（10タイプ）」セクションとして追記した。
これにより：
- generator.py が character.md を読み込むだけで各タイプの意図を理解して生成できる
- 新タイプ追加・変更時は character.md を更新するだけで全体に反映される

### 重み変更の根拠

- relatable（15）とbefore_after（14）を最高重みに：共感系・対比系がXでもっともエンゲージメントを生みやすい
- promoは5に維持：有益情報95%・ツール紹介5%のバランスを保つ
- seasonalを10に設定：日付連動ゆえ外れる可能性もあるが、タイムリー性の価値を評価

### 教訓

**投稿内容の質だけでなく「届け方のフォーマット」がエンゲージメントを決定する。**
同じ情報でも「教科書的な解説」より「あるある」「NG→OK」「クイズ」で包んだほうが読まれる。
コンテンツタイプはアカウント成長フェーズに合わせて定期的に見直すべき。
