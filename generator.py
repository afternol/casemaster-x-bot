...
- ❌ NGパターン：実際に受験生がやりがちな"リアルな発言・行動"を「」形式の具体的な台詞や行動で描写。「→ 〇〇になる」「→ 〇〇となる」のように矢印で悪影響を続ける
- ✅ OKパターン：正しいアプローチを同じく「」形式の具体的な台詞・行動で示す。「「○○という理解でよいですか？」」「「ゴールは〇〇か〇〇か確認させてください」」のように実際に口に出せる言葉レベルで
- なぜNGか・なぜOKかを視点の鋭い1〜2行で添える（「面接官が見ているのは〜ではなく〜」のような逆説的フレーズが効果的）
- 絵文字1〜2個
- 構造例：【フック1行】→【❌NG具体台詞・行動 + 矢印で影響描写】→【✅OK具体台詞・行動】→【洞察・理由1〜2行】→【ハッシュタグ】
- ハッシュタグは「#ケース面接」「#コンサル就活」「#就活対策」から1〜2個"""
    return _call_claude(prompt)
